...
MAX_BRIEF_BYTES=100_000; MAX_STREAM_BYTES=16_000_000
...
PYTEST_CACHE_FILES=frozenset({"README.md",".gitignore","CACHEDIR.TAG","v/cache/lastfailed",
                               "v/cache/nodeids","v/cache/stepwise"})

def _is_cache_artifact(rel:str)->bool:
    """Only generated bytecode and pytest's own fixed cache layout qualify."""
    parts=rel.split("/")
    if rel.endswith(".pyc"): return True
    if ".pytest_cache" in parts:
        return "/".join(parts[parts.index(".pytest_cache")+1:]) in PYTEST_CACHE_FILES
    return False

def _unlink_beneath(tree:Path,rel:str)->None:
    """Unlink tree/rel through directory handles opened without following links.

    Each component is opened relative to its parent's handle with O_NOFOLLOW,
    so a directory swapped for a symlink at any moment makes the open fail
    instead of redirecting the unlink outside the tree."""
    parts=rel.split("/")
    flags=os.O_RDONLY|getattr(os,"O_DIRECTORY",0)|getattr(os,"O_NOFOLLOW",0)
    fd=os.open(tree,flags)
    try:
        for part in parts[:-1]:
            if part in ("","." ,".."): return
...
def _drop_caches(tree:Path,env)->None:
    """Unlink ignored, untracked cache files a test run always leaves.

    `git ls-files --others --ignored` names only untracked files, so nothing
    git tracks is ever touched; its NUL-delimited output is read as exact
    bytes (never stripped). Each file must match a generated-artifact shape;
    anything else in a cache directory stays and still refuses the job.
    Removal goes through no-follow directory handles, and only regular files
    are unlinked. Platforms without dir_fd support skip cleanup, leaving the
    unchanged refusal in force."""
    if (not hasattr(os,"O_NOFOLLOW") or os.unlink not in os.supports_dir_fd
            or os.open not in os.supports_dir_fd or os.lstat not in os.supports_dir_fd): return
    r=_run(_git_argv("ls-files","-z","--others","--ignored","--exclude-standard"),cwd=tree,env=env,timeout=30)
    if r.returncode: raise TaskError("git prerequisite failed")
    for raw in r.stdout.split(b"\0"):
        if not raw: continue
        try: rel=os.fsdecode(raw)
        except UnicodeDecodeError: continue
        if not _is_cache_artifact(rel): continue
        try: _unlink_beneath(tree,rel)
        except OSError: continue

def _patch(tree:Path,base:str,env):
    _drop_caches(tree,env)
...
def _pinned_python(command:list[str])->list[str]:
    """Run `python`/`python3` verification with this worker's own interpreter.

    The policy admits only `python -m pytest|unittest`, but a bare name was
    resolved on the worker's PATH: `python` does not exist on stock macOS
    (sandbox-exec exits 71) and `python3` found /usr/bin's older runtime,
    not the one the project and its tests use. Pinning to sys.executable
    makes verification run where the harness itself runs."""
    if command and command[0] in ("python","python3") and sys.executable:
...
def _sandboxed(command:list[str],tree:Path,scratch:Path,env:dict[str,str],timeout:int,
    ...
    command=_pinned_python(command)
...
def _sandbox_exec_confined(command:list[str],tree:Path,scratch:Path,env:dict[str,str],timeout:int,
    ...
    # A pinned interpreter running inside a virtual environment reads its
    # packages from that environment, not from the resolved base binary.
    if command and command[0]==sys.executable and sys.prefix!=sys.base_prefix:
        read_roots.append(Path(sys.prefix).resolve())
