...
**OPEN, ratified as direction, not built:**
- VERIFY-01, VERIFY-03, VERIFY-07. A non-activating `--candidate` setup mode
  ...
  version-bound PASS. VERIFY-03 is the ordering defect: setup promotes a pin
  BEFORE re-measurement.
- VERIFY-02, VERIFY-04. `config/local.json` is a gitignored fragment with no
  `config_version`, and `config.load` layers broker plus local ONLY when no
  explicit path is given, so `--config` measures something the runtime never
  uses.
